...
/* 
 * RISC-V mcause (Machine Cause) CSR Mask Definitions
 * This header defines bitmasks to distinguish between Interrupts and Exception 
 * Causes within the RISC-V Machine-mode Cause Register (mcause).
 *
 * NOTE:
 * - These macros automatically scale to 32-bit (RV32) or 64-bit (RV64) 
 *   based on the compiler-defined '__riscv_xlen' macro.
 * - Ensure that your RISC-V GCC compiler flags (e.g., -march and -mabi) match 
 *   the exact hardware XLEN of the Rocket Chip generated in the Zynq PL fabric.
 *
 * WARN:
 * - DO NOT use or compile this header when targeting the Zynq-7000 hard 
 *   Processing System (PS) ARM Cortex-A9 cores. 
 * - ARMv7-A handles interrupts and exceptions through a completely different 
 *   vector table and coprocessor system (CP15), and has no concept of 
 *   RISC-V Control and Status Registers (CSRs) like 'mcause'.
 */
